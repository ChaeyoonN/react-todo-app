...
const Join = () => {
  ...
  // 검증 메시지에 대한 상태변수 관리
  // 입력값과 메시지는 따로 상태 관리(메시지는 벡엔드로 보내줄 필요 없음.)
  // 메시지 영역이 각 입력창마다 있기 때문에 객체를 활용해서 한 번에 관리.
  ...
  // 이름 입력창 체인지 이벤트 핸들러
  const nameHandler = (e) => {
    const nameRegex = /^[가-힣]{2,5}$/; // 한글 2~5자
    ...
    if (!inputValue) {
      // 입력값이 undefined, null, NaN, '' 이라면-> false
      msg = '유저 이름은 필수입니다.';
  ...
  const passwordHandler = (e) => {
    ...
    document.getElementById('password-check').value = ''; // 렌더링 끝난후에 수행되는 함수이므로 지목 가능.
